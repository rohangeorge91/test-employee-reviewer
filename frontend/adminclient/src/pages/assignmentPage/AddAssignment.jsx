...
import { fetchAllMembers, fetchAllAssessmentYear, onChangeReviewAssessment, resetReviewAssessment, editReviewAssessment, addReviewAssessment } from './action';
...
const mapDispatchToProps = (dispatch) => {
	return {
		fetchAllMembers: () => {
			return dispatch(fetchAllMembers()).then((members) => {
				const userId = members[0].userId;
				dispatch(onChangeReviewAssessment('reviewee.userId', userId));
				dispatch(onChangeReviewAssessment('reviewer.userId', userId))
				return members;
			});
		},
		fetchAllYears: () => {
			return dispatch(fetchAllAssessmentYear()).then((years) => {
				const year = years[0].year;
				dispatch(onChangeReviewAssessment('assessmentYear', year));
				return years;
			});
		},
		onChange: (name, value) => {
		...
		onSubmit: (e, reviewAssessment) => {
			e.preventDefault();
			return dispatch(addReviewAssessment(reviewAssessment)).then(() => {
				goBack();
			});
		},
		reset: () => {
